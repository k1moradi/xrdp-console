#!/usr/bin/env python3
"""Run XInput2 root-motion tracking against authenticated Xvfb."""
...
def main() -> int:
    ...
    auth_directory = tempfile.TemporaryDirectory(
        prefix="xrdp-console-xauthority-pointer-"
    )
    ...
    try:
        result = subprocess.run(
            [sys.argv[1]], env=environment, capture_output=True, text=True,
            check=False,
        )
        if result.returncode != 0:
            raise AssertionError(
                "XInput2 pointer-position integration failed with "
