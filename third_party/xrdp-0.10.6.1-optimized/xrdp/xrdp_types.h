...
/* The VNC proxy path is the only path which receives an existing WM screen. */
#define XRDP_MM_IS_VNC(mm) \
    (((mm)->code == XVNC_SESSION_CODE) || \
     ((mm)->code == XVNC_UDS_SESSION_CODE))
...
struct xrdp_mm
{
    ...
    /* Experimental VNC/GFX controls. Disabled unless explicitly enabled. */
    int vnc_gfx_profile;
    int vnc_gfx_backpressure;
    int vnc_gfx_max_dirty_rects;
    int vnc_gfx_async_planar;
    int vnc_gfx_async_pending;
