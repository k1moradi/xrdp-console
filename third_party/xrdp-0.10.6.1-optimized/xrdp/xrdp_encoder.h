...
/* One tightly-packed dirty rectangle for experimental VNC planar encoding. */
struct xrdp_enc_vnc_planar_rect
{
    int left;
    int top;
    int width;
...
/*
 * The main thread owns the WM screen. For asynchronous planar compression it
 * snapshots only the dirty rectangles into pixels and hands this immutable
 * job to the existing encoder worker. RDPGFX network writes remain on the
 * main thread.
 */
struct xrdp_enc_vnc_planar
{
    struct xrdp_enc_vnc_planar_rect *rects;
    int rect_count;
    char *pixels;
    int pixels_bytes;
    int surface_id;
...
#define ENC_DONE_FLAGS_VNC_PLANAR_BIT 2
...
#define ENC_FLAGS_VNC_PLANAR_BIT 1
...
struct xrdp_enc_data
{
    ...
    union _u
    {
        ...
        struct xrdp_enc_vnc_planar vnc_planar;
    } u;
};
