...
#define VNC_GFX_PLANAR_BYTES (32 * 1024)
...
static int
process_enc_vnc_planar(struct xrdp_encoder *self, XRDP_ENC_DATA *enc);
...
xrdp_enc_data_destructor(void *item, void *closure)
{
    ...
    if (ENC_IS_BIT_SET(enc->flags, ENC_FLAGS_VNC_PLANAR_BIT))
    {
        g_free(enc->u.vnc_planar.rects);
        g_free(enc->u.vnc_planar.pixels);
    }
    else if (ENC_IS_BIT_SET(enc->flags, ENC_FLAGS_GFX_BIT))
    {
...
process_enc_vnc_planar(struct xrdp_encoder *self, XRDP_ENC_DATA *enc)
{
    const struct xrdp_enc_vnc_planar *job;
    struct xrdp_enc_vnc_planar_rect *rect;
    ...
    int comp_bytes;
    int first_tile = 1;
    int emitted_completion = 0;
    int error = 0;
    int profile_start_ms;
    long long profile_raw_bytes = 0;
    long long profile_comp_bytes = 0;

    job = &enc->u.vnc_planar;
    profile_start_ms = job->profile ? g_time3() : 0;
    tile_pixels = g_new(char, VNC_GFX_PLANAR_BYTES);
    make_stream(comp_s);
    make_stream(temp_s);
    if (tile_pixels == NULL || comp_s == NULL || temp_s == NULL)
    {
        error = 1;
        goto finish;
    }
    init_stream(comp_s, VNC_GFX_PLANAR_BYTES);
    init_stream(temp_s, VNC_GFX_PLANAR_BYTES);
    if (comp_s->data == NULL || temp_s->data == NULL)
    {
        error = 1;
        goto finish;
    }

    for (rect_index = 0; rect_index < job->rect_count && error == 0;
            ++rect_index)
    {
        rect = job->rects + rect_index;
        if (rect->width < 1 || rect->height < 1)
        {
            continue;
        }
        if (rect->data_offset < 0 ||
                (long long)rect->data_offset +
                (long long)rect->width * rect->height * 4 >
                job->pixels_bytes)
        {
            LOG(LOG_LEVEL_ERROR,
                "process_enc_vnc_planar: invalid snapshot bounds");
            error = 1;
            break;
        }
        ...
        if (rect->width < 64)
        {
            ...
            tile_height = 4096 / tile_width;
        }
        else if (rect->height < 64)
        {
            tile_height = rect->height;
            tile_width = 4096 / tile_height;
        }
        else
        {
            tile_width = 64;
        ...
        while (tile_width * tile_height < 4096)
        {
        ...
        for (yoffset = 0; yoffset < rect->height && error == 0;
                yoffset += tile_height)
        {
            bheight = MIN(rect->height - yoffset, tile_height);
            for (xoffset = 0; xoffset < rect->width; xoffset += tile_width)
            {
                bwidth = MIN(rect->width - xoffset, tile_width);
                src8 = job->pixels + rect->data_offset +
                       (yoffset * rect->width + xoffset) * 4;
                dst8 = tile_pixels + (bheight - 1) * bwidth * 4;
                for (row_index = 0; row_index < bheight; ++row_index)
                {
                ...
                comp_s->end = comp_s->data + VNC_GFX_PLANAR_BYTES;
                temp_s->p = temp_s->data;
                temp_s->end = temp_s->data + VNC_GFX_PLANAR_BYTES;
                lines = libxrdp_planar_compress(tile_pixels, bwidth, bheight,
                                                comp_s, 32,
                                                VNC_GFX_PLANAR_BYTES,
                                                bheight - 1, temp_s,
                                                0, 0x10);
                comp_s->end = comp_s->p;
                comp_s->p = comp_s->data;
                ...
                {
                    LOG(LOG_LEVEL_ERROR,
                        "process_enc_vnc_planar: lines(%d) != height(%d)",
                        lines, bheight);
                    error = 1;
                    break;
                }
                ...
                comp_data = g_new(char, comp_bytes);
                enc_done = g_new0(XRDP_ENC_DATA_DONE, 1);
                if (comp_data == NULL || enc_done == NULL)
                {
                    g_free(comp_data);
                    g_free(enc_done);
                    error = 1;
                ...
                g_memcpy(comp_data, comp_s->data, comp_bytes);
                enc_done->comp_bytes = comp_bytes;
                enc_done->comp_pad_data = comp_data;
                enc_done->enc = enc;
                enc_done->continuation = !first_tile;
                enc_done->x = rect->left + xoffset - job->origin_x;
                enc_done->y = rect->top + yoffset - job->origin_y;
                enc_done->cx = bwidth;
                enc_done->cy = bheight;
                enc_done->frame_id = job->frame_id;
                enc_done->last = rect_index == job->rect_count - 1 &&
                                 yoffset + bheight >= rect->height &&
                                 xoffset + bwidth >= rect->width;
                ENC_SET_BIT(enc_done->flags,
...
finish:
    /*
     * Always return one final completion on failure (or an empty job), so the
     * main thread can clear vnc_gfx_async_pending and release the snapshot.
     */
    if (error != 0 || !emitted_completion)
    {
        enc_done = g_new0(XRDP_ENC_DATA_DONE, 1);
        if (enc_done != NULL)
        {
            enc_done->enc = enc;
            enc_done->last = 1;
            enc_done->continuation = emitted_completion;
            enc_done->frame_id = job->frame_id;
            ENC_SET_BIT(enc_done->flags, ENC_DONE_FLAGS_VNC_PLANAR_BIT);
            tc_mutex_lock(self->mutex);
            fifo_add_item(self->fifo_processed, enc_done);
            tc_mutex_unlock(self->mutex);
            g_set_wait_obj(self->xrdp_encoder_event_processed);
    ...
    {
        LOG(LOG_LEVEL_INFO,
            "VNC_GFX_PROFILE async_encode frame=%d rects=%d raw_bytes=%lld "
            "comp_bytes=%lld elapsed_ms=%d error=%d",
            job->frame_id, job->rect_count, profile_raw_bytes,
            profile_comp_bytes, MAX(0, g_time3() - profile_start_ms), error);
    }
    g_free(tile_pixels);
    free_stream(comp_s);
    free_stream(temp_s);
    ...
    while (cont)
    {
        ...
        if (g_is_wait_obj_set(event_to_proc))
        {
            ...
            while (enc != 0)
            {
                ...
                if (ENC_IS_BIT_SET(enc->flags, ENC_FLAGS_VNC_PLANAR_BIT))
                {
                    process_enc_vnc_planar(self, enc);
                }
                else
                {
