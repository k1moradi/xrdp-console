...
xrdp_mm_create(struct xrdp_wm *owner)
{
    ...
    {
        const char *vnc_gfx_profile = g_getenv("XRDP_VNC_GFX_PROFILE");
        const char *vnc_gfx_backpressure =
            g_getenv("XRDP_VNC_GFX_BACKPRESSURE");
        const char *max_dirty_rects =
            g_getenv("XRDP_VNC_GFX_MAX_DIRTY_RECTS");
        const char *async_planar =
            g_getenv("XRDP_VNC_ASYNC_PLANAR");

        self->vnc_gfx_profile = vnc_gfx_profile != NULL &&
                                g_text2bool(vnc_gfx_profile);
        self->vnc_gfx_backpressure = vnc_gfx_backpressure != NULL &&
                                     g_text2bool(vnc_gfx_backpressure);
        self->vnc_gfx_max_dirty_rects = 0;
        if (max_dirty_rects != NULL)
        {
            self->vnc_gfx_max_dirty_rects =
                MIN(32, MAX(0, g_atoi(max_dirty_rects)));
        }
        self->vnc_gfx_async_planar = async_planar != NULL &&
                                     g_text2bool(async_planar);
        self->vnc_gfx_async_pending = 0;
    }
...
/*****************************************************************************/
static int
xrdp_mm_vnc_gfx_frame_slot_available(const struct xrdp_mm *self)
{
    const struct xrdp_encoder *encoder;

    if (!XRDP_MM_IS_VNC(self) || self->wm->client_info->gfx == 0)
    {
        return 1;
    }

    if (self->vnc_gfx_async_planar && self->vnc_gfx_async_pending)
    {
        /* Keep newly dirtied pixels coalescing while the worker is busy. */
        return 0;
    }
    if (!self->vnc_gfx_backpressure)
    {
        return 1;
    }

    encoder = self->encoder;
    if (encoder == NULL || encoder->gfx_ack_off ||
            encoder->frames_in_flight <= 0 || encoder->frame_id_client <= 0)
    {
        /*
         * Do not throttle until this client has acknowledged at least one
         * numbered planar frame. This preserves compatibility with clients
         * which negotiate GFX but never send frame acknowledgements.
         */
        return 1;
    }

    /*
     * The VNC/WM-screen planar path is synchronous and historically uses
     * frame id 1 for every RDPGFX frame, bypassing the encoder's normal
     * frame-in-flight backpressure. Keep dirty pixels coalescing in the WM
     * screen while the client is at its configured in-flight limit.
     */
    return encoder->frame_id_server - encoder->frame_id_client <
           encoder->frames_in_flight;
}

/******************************************************************************
 * Sends planar tiles for one rectangle inside an already-open RDPGFX frame.
 * Scratch buffers are owned by the caller and reused for all dirty rectangles
 * in the logical frame so region-preserving mode does not add per-rectangle
 * heap allocation.
 */
static int
xrdp_mm_egfx_send_planar_rect_data(struct xrdp_mm *self,
                                   struct xrdp_bitmap *bitmap,
                                   const struct xrdp_rect *rect,
                                   int surface_id, int x, int y,
                                   char *pixels, struct stream *comp_s,
                                   struct stream *temp_s,
                                   long long *profile_raw_bytes,
                                   long long *profile_comp_bytes)
{
    ...
    LOG_DEVEL(LOG_LEVEL_TRACE,
              "xrdp_mm_egfx_send_planar_rect_data: cx %d cy %d", cx, cy);
    LOG_DEVEL(LOG_LEVEL_TRACE,
              "xrdp_mm_egfx_send_planar_rect_data: left %d top %d right %d "
    ...
    for (yindex = rect->top; yindex < rect->bottom; yindex += cy)
    {
        ...
        for (xindex = rect->left; xindex < rect->right; xindex += cx)
        {
            ...
            LOG_DEVEL(LOG_LEVEL_TRACE,
                      "xrdp_mm_egfx_send_planar_rect_data: xindex %d "
            ...
            {
                LOG(LOG_LEVEL_INFO,
                    "xrdp_mm_egfx_send_planar_rect_data: "
                    "lines(%d) != bheight(%d) error", lines, bheight);
            }
            else
            {
                comp_bytes = (int)(comp_s->end - comp_s->data);
                if (profile_raw_bytes != NULL)
                {
                    *profile_raw_bytes += (long long)bwidth * bheight * 4;
                }
                if (profile_comp_bytes != NULL)
                {
                    *profile_comp_bytes += comp_bytes;
                }
                LOG_DEVEL(LOG_LEVEL_TRACE,
                          "xrdp_mm_egfx_send_planar_rect_data: lines %d "
...
/*****************************************************************************/
static int
xrdp_mm_egfx_schedule_planar_rects(struct xrdp_mm *self,
                                   struct xrdp_bitmap *bitmap,
                                   const struct xrdp_rect *rects,
                                   int rect_count, int surface_id,
                                   int x, int y)
{
    XRDP_ENC_DATA *enc;
    struct xrdp_enc_vnc_planar *job;
    ...
    if (self->encoder == NULL || rect_count < 1 ||
            self->vnc_gfx_async_pending)
    {
        return self->vnc_gfx_async_pending ? 0 : 1;
    }

    for (rect_index = 0; rect_index < rect_count; ++rect_index)
    {
        width = rects[rect_index].right - rects[rect_index].left;
        height = rects[rect_index].bottom - rects[rect_index].top;
        if (width < 1 || height < 1)
        {
            continue;
        }
        total_bytes += (long long)width * height * 4;
        if (total_bytes > INT_MAX)
        {
            LOG(LOG_LEVEL_ERROR,
                "xrdp_mm_egfx_schedule_planar_rects: snapshot too large");
            return 1;
        }
    }
    if (total_bytes < 1)
    ...
    ENC_SET_BIT(enc->flags, ENC_FLAGS_VNC_PLANAR_BIT);
    job = &enc->u.vnc_planar;
    job->rects = g_new0(struct xrdp_enc_vnc_planar_rect, rect_count);
    job->pixels = g_new(char, (int)total_bytes);
    if (job->rects == NULL || job->pixels == NULL)
    {
        g_free(job->rects);
        g_free(job->pixels);
        g_free(enc);
    ...
    for (rect_index = 0; rect_index < rect_count; ++rect_index)
    {
        width = rects[rect_index].right - rects[rect_index].left;
        height = rects[rect_index].bottom - rects[rect_index].top;
        job->rects[rect_index].left = rects[rect_index].left;
        job->rects[rect_index].top = rects[rect_index].top;
        job->rects[rect_index].width = width;
        job->rects[rect_index].height = height;
        job->rects[rect_index].data_offset = data_offset;
        if (width < 1 || height < 1)
        {
            continue;
        }

        src = bitmap->data + bitmap->line_size * rects[rect_index].top +
              rects[rect_index].left * 4;
        dst = job->pixels + data_offset;
        for (row_index = 0; row_index < height; ++row_index)
        {
            g_memcpy(dst, src, width * 4);
            src += bitmap->line_size;
            dst += width * 4;
        }
        data_offset += width * height * 4;
    }

    job->rect_count = rect_count;
    job->pixels_bytes = data_offset;
    job->surface_id = surface_id;
    job->origin_x = x;
    job->origin_y = y;
    job->frame_id = self->vnc_gfx_backpressure ?
                    self->encoder->frame_id_server + 1 : 1;
    job->profile = self->vnc_gfx_profile;

    self->vnc_gfx_async_pending = 1;
    tc_mutex_lock(self->encoder->mutex);
    fifo_add_item(self->encoder->fifo_to_proc, enc);
    tc_mutex_unlock(self->encoder->mutex);
    g_set_wait_obj(self->encoder->xrdp_encoder_event_to_proc);

    if (self->vnc_gfx_profile)
    {
        LOG(LOG_LEVEL_INFO,
            "VNC_GFX_PROFILE async_schedule frame=%d rects=%d "
            "snapshot_bytes=%d",
            job->frame_id, rect_count, data_offset);
    }
    return 0;
}

/******************************************************************************
 * Sends one logical planar frame containing one or more dirty rectangles.
 */
static int
xrdp_mm_egfx_send_planar_rects(struct xrdp_mm *self,
                               struct xrdp_bitmap *bitmap,
                               const struct xrdp_rect *rects, int rect_count,
                               int surface_id, int x, int y)
{
    ...
    int frame_id;
    int rect_index;
    int profile_start_ms = 0;
    long long profile_raw_bytes = 0;
    long long profile_comp_bytes = 0;
    int rv = 0;
    ...
    if (self->vnc_gfx_async_planar && XRDP_MM_IS_VNC(self) &&
            self->encoder != NULL)
    {
        return xrdp_mm_egfx_schedule_planar_rects(self, bitmap, rects,
                                                  rect_count, surface_id,
                                                  x, y);
    }

    LOG_DEVEL(LOG_LEVEL_TRACE,
              "xrdp_mm_egfx_send_planar_rects: surface_id %d rect_count %d "
              "x %d y %d", surface_id, rect_count, x, y);
    if (self->vnc_gfx_profile && XRDP_MM_IS_VNC(self))
    {
        profile_start_ms = g_time3();
    }
    ...
    /*
     * Number VNC planar frames so RDPGFX acknowledgements can apply real
     * backpressure. Keep the historical frame id for other WM-screen users.
     * Commit a VNC id only after ENDFRAME is sent successfully.
     */
    frame_id = self->encoder != NULL && XRDP_MM_IS_VNC(self) &&
               self->vnc_gfx_backpressure ?
               self->encoder->frame_id_server + 1 : 1;
    if (xrdp_egfx_send_frame_start(self->egfx, frame_id, 0) != 0)
    {
        LOG(LOG_LEVEL_INFO, "xrdp_mm_egfx_send_planar_rects: "
    ...
    for (rect_index = 0; rect_index < rect_count; ++rect_index)
    {
        rv = xrdp_mm_egfx_send_planar_rect_data(
                 self, bitmap, rects + rect_index, surface_id, x, y,
                 pixels, comp_s, temp_s,
                 self->vnc_gfx_profile ? &profile_raw_bytes : NULL,
                 self->vnc_gfx_profile ? &profile_comp_bytes : NULL);
        if (rv != 0)
        {
            goto cleanup;
        }
    ...
    else if (self->encoder != NULL && XRDP_MM_IS_VNC(self) &&
             self->vnc_gfx_backpressure)
    {
        self->encoder->frame_id_server = frame_id;
    }

cleanup:
    if (self->vnc_gfx_profile && XRDP_MM_IS_VNC(self))
    {
        const int elapsed_ms = MAX(0, g_time3() - profile_start_ms);
        const int in_flight = self->encoder == NULL ? 0 :
                              self->encoder->frame_id_server -
                              self->encoder->frame_id_client;
        LOG(LOG_LEVEL_INFO,
            "VNC_GFX_PROFILE encode frame=%d rects=%d raw_bytes=%lld "
            "comp_bytes=%lld elapsed_ms=%d in_flight=%d",
            frame_id, rect_count, profile_raw_bytes, profile_comp_bytes,
            elapsed_ms, in_flight);
    }
    g_free(pixels);
    free_stream(comp_s);
    free_stream(temp_s);
...
/******************************************************************************
 * Sends a region using separate dirty rectangles when the opt-in rectangle
 * limit can represent it cheaply. Fragmented regions fall back to the historic
 * bounding-box behavior rather than creating an unbounded command stream.
 */
static int
xrdp_mm_egfx_send_planar_region(struct xrdp_mm *self,
                                ...
                                int surface_id, int x, int y)
{
    struct xrdp_rect rects[32];
    struct xrdp_rect extra_rect;
    struct xrdp_rect bounds;
    int rect_count = 0;
    int max_dirty_rects = self->vnc_gfx_max_dirty_rects;

    if (max_dirty_rects < 1)
    {
        if (xrdp_region_get_bounds(region, &bounds) != 0)
        {
            return 0;
        }
        return xrdp_mm_egfx_send_planar_rects(self, bitmap, &bounds, 1,
                                              surface_id, x, y);
    }

    while (rect_count < max_dirty_rects &&
            xrdp_region_get_rect(region, rect_count,
                                 rects + rect_count) == 0)
    {
        ++rect_count;
    }
    ...
    if (xrdp_region_get_rect(region, rect_count, &extra_rect) == 0)
    {
        if (xrdp_region_get_bounds(region, &bounds) != 0)
        {
            return 0;
        ...
        {
            LOG(LOG_LEVEL_INFO,
                "VNC_GFX_PROFILE region fallback=bounds limit=%d",
                max_dirty_rects);
        }
        return xrdp_mm_egfx_send_planar_rects(self, bitmap, &bounds, 1,
                                              surface_id, x, y);
    ...
    {
        LOG(LOG_LEVEL_INFO,
            "VNC_GFX_PROFILE region mode=rects rects=%d limit=%d",
            rect_count, max_dirty_rects);
    }
    return xrdp_mm_egfx_send_planar_rects(self, bitmap, rects, rect_count,
                                          surface_id, x, y);
}
...
xrdp_mm_egfx_send_planar_bitmap(struct xrdp_mm *self,
                                ...
                                int x, int y)
{
    return xrdp_mm_egfx_send_planar_rects(self, bitmap, rect, 1,
                                          surface_id, x, y);
}
                      ...
                      tbus *write_objs, int *wcount, int *timeout)
{
    ...
    if (self->wm->screen_dirty_region != NULL)
    {
        if (xrdp_region_not_empty(self->wm->screen_dirty_region) &&
                xrdp_mm_vnc_gfx_frame_slot_available(self))
        {
            int now = g_time3();
...
/*****************************************************************************/
static void
xrdp_mm_process_vnc_planar_done(struct xrdp_mm *self,
                                XRDP_ENC_DATA_DONE *enc_done)
{
    XRDP_ENC_DATA *enc = enc_done->enc;
    struct xrdp_egfx_rect gfx_rect;
    int frame_has_data;
    int end_ok = 0;

    frame_has_data = enc_done->continuation || enc_done->comp_bytes > 0;
    if (!enc_done->continuation && enc_done->comp_bytes > 0)
    {
        if (xrdp_egfx_send_frame_start(self->egfx, enc_done->frame_id, 0) != 0)
        {
            LOG(LOG_LEVEL_ERROR,
                "xrdp_mm_process_vnc_planar_done: frame start failed");
        }
    }

    if (enc_done->comp_bytes > 0)
    {
        gfx_rect.x1 = enc_done->x;
        gfx_rect.y1 = enc_done->y;
        gfx_rect.x2 = enc_done->x + enc_done->cx;
        gfx_rect.y2 = enc_done->y + enc_done->cy;
        if (xrdp_egfx_send_wire_to_surface1(
                self->egfx, enc->u.vnc_planar.surface_id,
                XR_RDPGFX_CODECID_PLANAR, XR_PIXEL_FORMAT_XRGB_8888,
                &gfx_rect, enc_done->comp_pad_data,
                enc_done->comp_bytes) != 0)
        {
            LOG(LOG_LEVEL_ERROR,
                "xrdp_mm_process_vnc_planar_done: wire-to-surface failed");
        }
    }
    ...
    if (enc_done->last)
    {
        if (frame_has_data)
        {
            end_ok = xrdp_egfx_send_frame_end(self->egfx,
                                               enc_done->frame_id) == 0;
            if (!end_ok)
            {
                LOG(LOG_LEVEL_ERROR,
                    "xrdp_mm_process_vnc_planar_done: frame end failed");
            }
        }
        if (end_ok && self->vnc_gfx_backpressure)
        {
            self->encoder->frame_id_server = enc_done->frame_id;
        }
        self->vnc_gfx_async_pending = 0;
        g_free(enc->u.vnc_planar.rects);
        g_free(enc->u.vnc_planar.pixels);
        g_free(enc);
    }
...
xrdp_mm_process_enc_done(struct xrdp_mm *self)
{
    ...
    while (1)
    {
        ...
        if (ENC_IS_BIT_SET(enc_done->flags,
                           ENC_DONE_FLAGS_VNC_PLANAR_BIT))
        {
            xrdp_mm_process_vnc_planar_done(self, enc_done);
            continue;
        }
...
xrdp_mm_draw_dirty(struct xrdp_mm *self)
{
    ...
    if (self->vnc_gfx_profile && XRDP_MM_IS_VNC(self))
    {
        struct xrdp_rect dirty_rect;
        struct xrdp_rect dirty_bounds;
        long long dirty_pixels = 0;
        long long bounds_pixels = 0;
        int dirty_rect_count = 0;

        while (xrdp_region_get_rect(self->wm->screen_dirty_region,
                                    dirty_rect_count, &dirty_rect) == 0)
        {
            dirty_pixels += (long long)(dirty_rect.right - dirty_rect.left) *
                            (dirty_rect.bottom - dirty_rect.top);
            ++dirty_rect_count;
        }
        if (xrdp_region_get_bounds(self->wm->screen_dirty_region,
                                   &dirty_bounds) == 0)
        {
            bounds_pixels =
                (long long)(dirty_bounds.right - dirty_bounds.left) *
                (dirty_bounds.bottom - dirty_bounds.top);
        }
        LOG(LOG_LEVEL_INFO,
            "VNC_GFX_PROFILE dirty rects=%d dirty_pixels=%lld "
            "bounds_pixels=%lld amplification_milli=%lld",
            dirty_rect_count, dirty_pixels, bounds_pixels,
            dirty_pixels == 0 ? 0 : bounds_pixels * 1000 / dirty_pixels);
    }
    count = self->wm->client_info->display_sizes.monitorCount;
    if (count < 1)
    {
        rv = xrdp_mm_egfx_send_planar_region(self, self->wm->screen,
