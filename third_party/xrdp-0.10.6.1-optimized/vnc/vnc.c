...
send_update_request_for_resize_status(struct vnc *v)
{
    ...
    switch (v->resize_status)
    {
        ...
        default:
            ...
            if (v->suppress_output == 0)
            {
                request_width = v->server_layout.total_width;
                request_height = v->server_layout.total_height;
                out_uint8(s, RFB_C2S_FRAMEBUFFER_UPDATE_REQUEST);
                out_uint8(s, 0); /* incremental == 0 : Full update */
                out_uint16_be(s, 0);
                out_uint16_be(s, 0);
...
/******************************************************************************
 * Schedules the next bounded RAW strip for the incremental framebuffer parser.
 * One callback handles at most framebuffer_read_quantum bytes unless a single
 * scanline is larger. The VNC transport input buffer is 64 KiB, which covers
 * the largest supported 32-bpp scanline.
 */
static int
lib_framebuffer_incremental_schedule_raw(struct vnc *v)
{
    int rows_remaining;
    int rows_per_read;
    int read_bytes;

    rows_remaining = v->framebuffer_cy - v->framebuffer_raw_rows_done;
    if (rows_remaining <= 0 || v->framebuffer_raw_row_bytes <= 0)
    {
        return 1;
    }

    rows_per_read = v->framebuffer_read_quantum /
                    v->framebuffer_raw_row_bytes;
    rows_per_read = MAX(1, rows_per_read);
    rows_per_read = MIN(rows_per_read, rows_remaining);
    read_bytes = rows_per_read * v->framebuffer_raw_row_bytes;
    if (read_bytes > v->trans->in_s->size)
    {
        LOG(LOG_LEVEL_ERROR,
            "lib_framebuffer_incremental_schedule_raw: "
            "RAW strip %d exceeds transport buffer %d",
            read_bytes, v->trans->in_s->size);
        return 1;
    }

    v->framebuffer_parse_state = VNC_FB_WAIT_RAW_DATA;
    v->trans->header_size = read_bytes;
    return 0;
}
...
lib_framebuffer_incremental_finish_update(struct vnc *v)
{
    ...
    if (error == 0)
    {
        v->framebuffer_parse_state = VNC_FB_WAIT_MESSAGE_TYPE;
...
/******************************************************************************
 * Advances to the next rectangle, or finishes the logical framebuffer update.
 */
static int
lib_framebuffer_incremental_finish_rect(struct vnc *v)
{
    if (--v->framebuffer_rects_remaining > 0)
    {
        v->framebuffer_parse_state = VNC_FB_WAIT_RECT_HEADER;
        v->trans->header_size = 12;
        return 0;
    }
    return lib_framebuffer_incremental_finish_update(v);
}

/******************************************************************************
 * Processes a cursor payload using the same conversion as the blocking parser.
 */
static int
lib_framebuffer_incremental_cursor(struct vnc *v, int x, int y, int cx, int cy)
{
    char *d1;
    char *d2;
    char cursor_data[32 * (32 * 3)];
    ...
    g_memset(cursor_data, 0, sizeof(cursor_data));
    g_memset(cursor_mask, 0, sizeof(cursor_mask));
    cursor_bytes = cx * cy * get_bytes_per_pixel(v->server_bpp);
    mask_bytes = ((cx + 7) / 8) * cy;
    make_stream(s);
    init_stream(s, cursor_bytes + mask_bytes);
    error = trans_force_read_s(v->trans, s, cursor_bytes + mask_bytes);
    if (error == 0)
    {
        in_uint8p(s, d1, cursor_bytes);
        in_uint8p(s, d2, mask_bytes);
        for (row = 0; row < 32; ++row)
        {
            for (column = 0; column < 32; ++column)
            {
                pixel = get_pixel_safe(d2, column, 31 - row, cx, cy, 1);
                set_pixel_safe(cursor_mask, column, row, 32, 32, 1, !pixel);
                if (pixel)
                {
                    pixel = get_pixel_safe(d1, column, 31 - row,
                                           cx, cy, v->server_bpp);
                    split_color(pixel, &r, &g, &b,
                                v->server_bpp, v->palette);
                    pixel = make_color(r, g, b, 24);
                    set_pixel_safe(cursor_data, column, row,
                                   32, 32, 24, pixel);
                }
            }
        }
...
/******************************************************************************
 * Processes one callback for the opt-in incremental steady-state framebuffer
 * parser. Large RAW rectangles are painted as bounded scanline strips so the
 * main xrdp thread returns to RDP input processing between strips.
 */
static int
lib_framebuffer_incremental_data(struct vnc *v, struct stream *s)
{
    struct vnc_screen_layout layout = {0};
    struct stream *payload_s;
    unsigned int encoding;
    int srcx;
    int srcy;
    int rows;
    ...
    switch (v->framebuffer_parse_state)
    {
        case VNC_FB_WAIT_UPDATE_HEADER:
            ...
            if (v->framebuffer_rects_remaining == 0)
            {
                error = lib_framebuffer_incremental_finish_update(v);
            }
            else
            {
                v->framebuffer_parse_state = VNC_FB_WAIT_RECT_HEADER;
        ...
        case VNC_FB_WAIT_RECT_HEADER:
            in_uint16_be(s, v->framebuffer_x);
            in_uint16_be(s, v->framebuffer_y);
            in_uint16_be(s, v->framebuffer_cx);
            in_uint16_be(s, v->framebuffer_cy);
            in_uint32_be(s, encoding);
            if (!framebuffer_update_size_ok(v->framebuffer_cx,
                                            v->framebuffer_cy))
            {
                LOG(LOG_LEVEL_ERROR,
                    "lib_framebuffer_incremental_data: Frame buffer too large");
                return 1;
            }
            ...
            {
                make_stream(payload_s);
                init_stream(payload_s, 4);
                error = trans_force_read_s(v->trans, payload_s, 4);
                if (error == 0)
                {
                    in_uint16_be(payload_s, srcx);
                    in_uint16_be(payload_s, srcy);
                    error = v->server_screen_blt(v,
                                                 v->framebuffer_x,
                                                 v->framebuffer_y,
                                                 v->framebuffer_cx,
                                                 v->framebuffer_cy,
            ...
            {
                error = lib_framebuffer_incremental_cursor(
                            v, v->framebuffer_x, v->framebuffer_y,
                            v->framebuffer_cx, v->framebuffer_cy);
                if (error == 0)
                {
                    error = lib_framebuffer_incremental_finish_rect(v);
            ...
            {
                init_single_screen_layout(v->framebuffer_cx,
                                          v->framebuffer_cy,
                                          &v->server_layout);
                error = resize_client_to_server(v, 1);
                if (error == 0)
            ...
            {
                layout.total_width = v->framebuffer_cx;
                layout.total_height = v->framebuffer_cy;
                error = read_extended_desktop_size_rect(v, &layout);
                if (error == 0 && v->framebuffer_x != 1 &&
                        !vnc_screen_layouts_equal(&v->server_layout, &layout))
                {
                    v->server_layout = layout;
                    log_screen_layout(LOG_LEVEL_INFO, "NewServerLayout",
                                      &v->server_layout);
            ...
            else
            {
                char text[256];
                g_sprintf(text,
                          "VNC error in incremental framebuffer encoding = %8.8x",
                          encoding);
                v->server_msg(v, text, 1);
                error = 1;
            }
        ...
        case VNC_FB_WAIT_RAW_DATA:
            bytes = (int)(s->end - s->data);
            if (v->framebuffer_raw_row_bytes <= 0 ||
                    bytes % v->framebuffer_raw_row_bytes != 0)
            {
                LOG(LOG_LEVEL_ERROR,
                    "lib_framebuffer_incremental_data: invalid RAW strip %d/%d",
                    bytes, v->framebuffer_raw_row_bytes);
                return 1;
            }
            rows = bytes / v->framebuffer_raw_row_bytes;
            error = v->server_paint_rect(
                        v,
                        v->framebuffer_x,
        ...
        case VNC_FB_WAIT_MESSAGE_TYPE:
        default:
            LOG(LOG_LEVEL_ERROR,
                "lib_framebuffer_incremental_data: invalid parser state %d",
                v->framebuffer_parse_state);
            error = 1;
            break;
    }
...
/******************************************************************************
 * Begins an incremental steady-state framebuffer update after the one-byte RFB
 * server message type has already been consumed.
 */
static int
...
lib_data_in(struct trans *trans)
{
    ...
    if (self->incremental_framebuffer &&
            self->framebuffer_parse_state != VNC_FB_WAIT_MESSAGE_TYPE)
    {
        error = lib_framebuffer_incremental_data(self, s);
    }
    else if (self->incremental_framebuffer &&
             self->resize_status == VRS_DONE &&
             (s->end - s->data) == 1 &&
             (unsigned char)s->data[0] == RFB_S2C_FRAMEBUFFER_UPDATE)
    {
        error = lib_framebuffer_incremental_begin(self);
    }
    else
    {
        error = lib_mod_process_message(self, s);
    }

    if (error != 0)
    {
        LOG(LOG_LEVEL_ERROR, "lib_data_in: VNC message processing failed");
        return 1;
    }
...
mod_init(void)
{
    ...
    v->dynamic_resizing = 1;
    {
        ...
        {
            v->framebuffer_read_quantum =
                MIN(65536, MAX(1024, g_atoi(read_quantum)));
        }
        v->framebuffer_parse_state = VNC_FB_WAIT_MESSAGE_TYPE;
    }
    vnc_clip_init(v);
