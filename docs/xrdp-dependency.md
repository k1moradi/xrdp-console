...
| `0015-xrdp-chansrv-strict-text-clipboard.patch` | strict text-only CLIPRDR formats with compatibility padding | Limits text format advertisement to Unicode, preserves upstream four-byte compatibility padding in clipboard capability/format/text messages, and flushes the X11 selection request without changing chansrv ownership. |
