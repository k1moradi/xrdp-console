...
| source tree | `build/_deps/xrdp-src` |
| build tree | `build/_deps/xrdp-build` |
...
The dependency build uses `-O3 -march=native` by default. Configure
...
| `0003-xrdp-input-first-transport-check.patch` | early transport check | Avoids backend work after the RDP transport has already disconnected. |

These are production compatibility changes, not benchmark knobs. The old
